// Runs the acquisition worker against the real TEBOS schema (all migrations,
// guards, RLS and audit triggers). Enabled by scripts/test-db.sh, which sets
// TEBOS_TEST_DB=1 and the libpq PG* variables for a fresh test database.
import pg from "pg";
...
describe.skipIf(!enabled)("acquisition worker against the TEBOS schema", () => {
  let pool: pg.Pool;
