...
| Connections | Org admins connect Resend: sender address and API key (stored in Supabase Vault and never shown again), an optional webhook signing secret, "Check now", disable and enable. The status shown is only what TEBOS's worker verified with the provider: "Connected" never comes from the browser. |
| Email actions | Choosing "Send transactional email" when proposing an action asks for the exact recipients, subject and message. They are checked before approval, and frozen once approval is requested. After approval, TEBOS's worker sends the email. The action page shows the wait for the worker and then for the provider's delivery confirmation, and offers no manual "start" or "verify" for provider actions. |
