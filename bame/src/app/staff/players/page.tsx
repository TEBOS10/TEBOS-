...
export default async function StaffPlayersPage() {
  ...
  const { data: players } = await supabase
    .from("players")
    .select("id, full_name, sport, package_tier, status, photo_url")
    .order("full_name");
                ...
                <div className="mt-3 grid gap-4 md:grid-cols-3 lg:grid-cols-4">
                  {tierPlayers.map((p) => (
                    ...
                    >
                      ...
                      <p className="mt-3 text-center text-sm font-medium">{p.full_name}</p>
                      <p className="text-center text-xs text-[var(--bame-muted)]">
